...
| `USR2` | Remote debug, see :mod:`celery.contrib.rdb`. |
...
!!! info "Number of processes (multiprocessing/prefork pool)"
...
!!! tip inline end "The ``celery`` command"
...
!!! question inline end "Soft, or hard?"
...
- The autoscaler adds more pool processes when there is work to do, and starts removing processes when the workload is low.
