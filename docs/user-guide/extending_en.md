...
<figcaption>
Bootsteps in the Worker and Consumer blueprints. Starting
...
<tr>
<td><code>app</code></td>
<td>The current app instance.</td>
</tr>
<tr>
<td><code>hostname</code></td>
<td>The workers node name (e.g., `worker1@example.com`)</td>
</tr>
<tr>
<td><code>blueprint</code></td>
<td>This is the worker :class:`~celery.bootsteps.Blueprint`.</td>
</tr>
<tr>
<td><code>hub</code></td>
<td>
Event loop object (:class:`~kombu.asynchronous.Hub`). You can use
this to register callbacks in the event loop.<br/><br/>
This is only supported by async I/O enabled transports (amqp, redis),
in which case the `worker.use_eventloop` attribute should be set.<br/><br/>
...
See :class:`celery.concurrency.base.BasePool`.<br/><br/>
...
<td>
:class:`~kombu.asynchronous.timer.Timer` used to schedule functions.<br/><br/>
...
worker restarts.<br/><br/>
This is only defined if the `statedb` argument is enabled.<br/><br/>
...
and shrink the number of processes in the pool.<br/><br/>
This is only defined if the `autoscale` argument is enabled.<br/><br/>
...
reload use code when the file-system changes.<br/><br/>
...
<td>
The current broker connection (:class:`kombu.Connection`).<br/><br/>
...
<td>
A :class:`@events.Dispatcher` object that can be used to send events.<br/><br/>
...
<td>
Worker to worker broadcast communication(:class:`~celery.worker.consumer.gossip.Gossip`).<br/><br/>
...
providing a :class:`~celery.events.state.Worker` instance.<br/>
...
(:class:`~celery.worker.heartbeat.Heart`).<br/><br/>
...
<td>
The :class:`kombu.Consumer` object used to consume task messages.<br/><br/>
...
`consume(tokens)` and `expected_time(tokens)`.<br/><br/>

TokenBucket implements the [token bucket algorithm](https://en.wikipedia.org/wiki/Token_bucket), but any algorithm
...
| Method                                                                                 | Description                                                                                                     |
|----------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------|
| `reset_rate_limits()`                                                                  | Updates the `task_buckets` mapping for all registered task types.                                               |
| `bucket_for_task(type, Bucket=TokenBucket)`                                            | Creates rate limit bucket for a task using its `task.rate_limit` attribute.                                     |
| `add_task_queue(name, exchange=None, exchange_type=None, routing_key=None, **options)` | Adds new queue to consume from. This will persist on connection restart.                                        |
| `cancel_task_queue(name)`                                                              | Stop consuming from queue by name. This will persist on connection restart.                                     |
| `apply_eta_task(request)`                                                              | Schedule ETA task to execute based on the `request.eta` attribute. (:class:`~celery.worker.request.Request`)    |
...
[setuptools entry-points](https://setuptools.readthedocs.io/en/latest/setuptools.html#dynamic-discovery-of-services-and-plugins).
...
| `add_reader(fd, callback, *args)`  | Add callback to be called when `fd` is readable.The callback will stay registered until explicitly removed using:meth:`hub.remove(fd) <hub.remove>`, or the file descriptor isautomatically discarded because it's no longer valid.Note that only one callback can be registered for any givenfile descriptor at a time, so calling `add` a second time will removeany callback that was previously registered for that file descriptor.A file descriptor is any file-like object that supports the `fileno`method, or it can be the file descriptor number (int).        |
| `add_writer(fd, callback, *args)`  | Add callback to be called when `fd` is writable. See also notes for :meth:`hub.add_reader` above.                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `remove(fd)`                       | Remove all callbacks for file descriptor `fd` from the loop.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
