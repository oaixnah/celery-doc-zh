...
but for the same reasons `pickle` is inherently insecure,
...
Using [Public-key cryptography](https://en.wikipedia.org/wiki/Public-key_cryptography) the `auth` serializer can verify the
