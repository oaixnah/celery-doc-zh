...
| `__call__(*args, **kwargs)`        | Applying an object supporting the calling API (e.g., `add(2, 2)`) means that the task will not be executed by a worker, but in the current process instead (a message won't be sent). |

??? example "Quick Cheat Sheet"

| Function                                       | Description                                                                                                |
|------------------------------------------------|------------------------------------------------------------------------------------------------------------|
| `delay(arg, kwarg=value)`                      | Star arguments shortcut to `.apply_async`. (`.delay(*args, **kwargs)` calls `.apply_async(args, kwargs)`). |
| `apply_async((arg,), {'kwarg': value})`        | -                                                                                                          |
| `apply_async(countdown=10)`                    | executes in 10 seconds from now.                                                                           |
| `apply_async(eta=now + timedelta(seconds=10))` | executes in 10 seconds from now, specified using `eta`.                                                    |
| `apply_async(countdown=60, expires=120)`       | executes in one minute from now, but expires after 2 minutes.                                              |
| `apply_async(expires=now + timedelta(days=2))` | expires in 2 days, set using :class:`~datetime.datetime`.                                                  |
| `apply_async(task_id=f'my_own_task_id')`       | sets the id of the task to my_own_task_id instead of a uuid that is normally generated.                    |
...
!!! warning
    ...
    See [Delivery Acknowledgement Timeout](https://www.rabbitmq.com/docs/consumers#acknowledgement-timeout) for more information.
...
exception that caused the retry to fail will be raised.<br/><br/>

A value of :const:`None` means it will retry forever.<br/><br/>
...
It will be ignored if not specified. Default is None (ignored).<br/><br/>
...
msgpack is a binary serialization format that's closer to JSON
...
!!! tip inline end "Automatic Pool Support"
...
| Option        | Description                                                                                                                           |
|---------------|---------------------------------------------------------------------------------------------------------------------------------------|
| `exchange`    | Name of exchange (or a :class:`kombu.entity.Exchange`) to send the message to.                                                        |
