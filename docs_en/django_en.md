...
A common solution is to use Django's [on_commit](https://docs.djangoproject.com/en/stable/topics/db/transactions/#django.db.transaction.on_commit) hook to trigger the task
...
1. Install the :pypi:`django-celery-results` library:

    ```console
    pip install django-celery-results
    ```

2. Add `django_celery_results` to `INSTALLED_APPS` in your Django project's :file:`settings.py`:
