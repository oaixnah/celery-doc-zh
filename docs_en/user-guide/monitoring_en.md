...
This will list all tasks that have been revoked.
...
This will list all tasks that are registered in the system.

#### inspect stats

Show worker statistics (see :ref:`worker-statistics`)
...
Show information about task(s) by id.
...
Migrate tasks from one broker to another (**EXPERIMENTAL**).
...
RabbitMQ ships with the [`rabbitmqctl(1)`](https://www.rabbitmq.com/man/rabbitmqctl.1.man.html) command,
...
!!! tip
    ...
    Adding the `-q` option to `rabbitmqctl(1)`_ makes the output easier to parse.
...
- [rabbitmq-munin](https://github.com/ask/rabbitmq-munin): Munin plug-ins for RabbitMQ.
- [celery_tasks](https://github.com/munin-monitoring/contrib/blob/master/plugins/celery/celery_tasks): Monitors the number of times each task type has
  been executed (requires `celerymon`).
- [celery_tasks_states](https://github.com/munin-monitoring/contrib/blob/master/plugins/celery/celery_tasks_states): Monitors the number of tasks in each state
  (requires `celerymon`).
